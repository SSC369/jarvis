...
| 002 | [Authentication](./002-authentication/) | P0, blocking | 5 Dev | in progress | Slice 1 (manual signup/sign-in) and slice 3 (Google, including avatar and sign-out) built and live-verified against the real Supabase project. Two real bugs found and fixed: `me` threw instead of returning `username: null`; Google avatar backfill for pre-existing accounts, user-confirmed working. Manual follow-ups remain: T-1.4 (register the two Auth Hooks) and T-1.13's SMTP/email-template config, both need dashboard access; T-3.5 needs one more Google sign-in against an email with an existing manual account to confirm FR-20's auto-link. Next up: slice 2 (password reset, approved, not started) | 2026-09-15 |
