...
| Term | Definition |
|---|---|
| Command Center | The primary Jarvis interface where the user types commands, converses, searches and retrieves. |
...
| Record | One structured item Jarvis has stored for the user: a task, reminder, expense, memory, event, goal, project or note. |
...
| Memory | Information the user has asked Jarvis to retain as durable context, distinct from a task or a note. |
| Forget | Deleting a memory at the user's instruction. Destructive, so it confirms. |
| Life Inbox | Frictionless capture where the user dumps input without categorising it, and Jarvis proposes the record type for review. |
| Classification | Jarvis deciding which record type a piece of plain-language input becomes. |
| Extraction | Jarvis pulling the structured fields for a record type out of natural-language input. |
| Personal search | Unified search across every record type, so the user need not know which category holds the answer. |
| Contextual intelligence | Jarvis understanding relationships between records, such as a task belonging to a project that serves a goal. |
| Proactive suggestion | Information Jarvis surfaces without being asked. Relevant, limited, actionable, non-intrusive. |
...
| Confirmation model | The rule for when Jarvis acts immediately, asks one question, or requires explicit confirmation. |
