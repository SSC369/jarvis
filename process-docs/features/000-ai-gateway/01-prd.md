...
Every capture in Slashit runs through a model. `/add-task Finish docs tomorrow`
...
The credential is the first. One provider key belongs to Slashit, not to users,
...
- **US-2.** As the person capturing, I am told plainly when the model cannot serve me and why, so that I do not think Slashit is broken or silently lost my input.
